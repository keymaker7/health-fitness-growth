...
export const MEASURE_TOOLS: MeasureTool[] = [
  {
    ...
    alsoAt: { label: "micro:bit로 줄넘기하기 (기기 필요)", url: "https://jumprope-fighter.netlify.app" },
    guide: {
      ready: [
  ...
  },
  {
    ...
    guide: {
      ready: [
        "바닥에 줄자를 뛰는 방향으로 곧게 깔아요 — 발구름선이 0m예요.",
        "태블릿은 옆에 두고 줄자 0m~3m 구간이 화면에 다 보이게 하세요. 필요하면 «뒷면 카메라로» 버튼으로 카메라를 바꿔요.",
      ],
      steps: [
        "«카메라 켜기»를 눌러요.",
        "화면 속 줄자 위 0m → 1m → 2m → 3m 지점을 순서대로 눌러요. (눈금 간격·점 개수는 바꿀 수 있어요)",
        "바닥에 초록 눈금이 그려져요 — 화면의 1m·2m 선이 실제 줄자 눈금과 겹치는지 눈으로 확인하세요.",
        "발구름선 뒤에 서서 잠깐 가만히 있으면 준비가 자동으로 잡히고, 뛰면 착지 지점으로 거리가 계산돼요. 선을 밟으면 파울로 표시돼요.",
        "세 번 뛰면 «최고 (파울 제외)» 값이 잡혀요 → «기록에 담기»를 눌러요.",
      ],
      tips: [
        "태블릿이나 카메라를 옮겼으면 «기준 다시 잡기»로 점을 다시 찍어야 정확해요.",
        "화면 눈금이 줄자와 어긋나 보이면 재지 말고 기준부터 다시 잡으세요 — 값이 이상하면 숫자 대신 «측정 오류»로 표시돼요.",
